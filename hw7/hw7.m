...
bfgs = @(gamma,dx,Hp) Hp + (gamma*gamma')/(gamma'*dx) - (Hp*dx*dx'*Hp)/(dx'*Hp*dx);

ffun = @(x_,df_,L_,dg_,g_) [ df_(1) + L_(1,1)*x_(1) + L_(1,2)*x_(2) - x_(3)*dg_(1);
                          df_(2) + L_(2,1)*x_(1) + L_(2,2)*x_(2) - x_(3)*dg_(2);
                          g_ + dg_(1)*x_(1) + dg_(2)*x_(2) ];
ffuns = @(x_,df_,L_) [ df_(1) + L_(1,1)*x_(1) + L_(1,2)*x_(2);
